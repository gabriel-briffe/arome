...
def fetch_wind_component_tiff(
    ...
    """
    Fetch U or V wind component data from the AROME API in TIFF format

    Parameters:
    -----------
    component_type : str
        Either 'U' or 'V' for the wind component
    time_value : str
    ...
    """
    if component_type not in ['U', 'V']:
        logger.error(f"Invalid component_type: {component_type}. Must be 'U' or 'V'")
        return False

    # Set default output filename if not provided
    if output_file is None:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        output_file = f"wind_{component_type.lower()}_component_{timestamp}.tiff"

    # Build the coverageId from the component type and reference time
    coverage_id = f"{component_type}_COMPONENT_OF_WIND__ISOBARIC_SURFACE___{ref_time_value}"
    ...
    logger.info(f"Requesting {component_type} wind component data from: {url}")
    ...
    while retry_count <= max_retries:
        try:
            logger.info(f"Fetching {component_type} wind component data for {time_value} at {pressure_value}hPa")
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Successfully saved {component_type} wind component TIFF to {output_file}")
    ...
    logger.error(f"Failed to fetch {component_type} wind component data after {max_retries} retries.")
    return False
